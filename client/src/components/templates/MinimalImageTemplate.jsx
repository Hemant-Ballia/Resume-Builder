            ...
            <div className="grid grid-cols-3">
                ...
                <aside className="col-span-1 border-r border-zinc-400 p-6 pt-0">
                    ...
                    <section className="mb-8">
                        ...
                        <div className="space-y-3 text-sm"> {/* margin thoda badha diya for better spacing */}
                            {data.personal_info?.phone && (
                                <div className="flex items-start gap-2">
                                    <Phone size={14} className="shrink-0 mt-[2px]" style={{ color: accentColor }} />
                                    <span className="break-all">{data.personal_info.phone}</span>
                                </div>
                            )}
                            {data.personal_info?.email && (
                                <div className="flex items-start gap-2">
                                    <Mail size={14} className="shrink-0 mt-[2px]" style={{ color: accentColor }} />
                                    <span className="break-all">{data.personal_info.email}</span>
                                </div>
                            )}
                            {data.personal_info?.location && (
                                <div className="flex items-start gap-2">
                                    <MapPin size={14} className="shrink-0 mt-[2px]" style={{ color: accentColor }} />
                ...
                <main className="col-span-2 p-8 pt-0">
                    ...
                    {data.professional_summary && (
                        <section className="mb-8">
                            ...
                            <p className="text-zinc-700 leading-relaxed text-sm"> {/* Added text-sm so it looks proportionate */}
                                {data.professional_summary}
                            </p>
                        </section>
                    )}
                            ...
                            <div className="space-y-6 mb-8">
                                {data.experience.map((exp, index) => (
                                    <div key={index}>
                                        ...
                                        {exp.description && (
                                            <ul className="list-disc list-outside ml-4 text-sm text-zinc-700 leading-relaxed space-y-1">
                            ...
                            <div className="space-y-4">
                                {data.project.map((project, index) => (
                                    <div key={index}>
                                        ...
                                        <p className="text-sm mb-1 font-medium" style={{ color: accentColor }} >
                                            {project.type}
                                        </p>
                                        {project.description && (
